...
import io.github.tlh.jmb.common.config.GithubConfig;
...
public class GithubController {
    ...
    public String callback(String code, String state, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (!StringUtils.isEmpty(code) && !StringUtils.isEmpty(state)) {
            ...
            Map respTokenMap = HttpUtil.doGet(tokenUrl, HashMap.class);
            // 调用方法从map中获得返回的--》 令牌
            String token = String.valueOf(respTokenMap.get("access_token"));

            //根据token发送请求获取登录人的信息  ，通过令牌去获得用户信息
            String userInfoUrl = String.format(githubConfig.getUserInfoUrl(), token);
            Map respUserMap = HttpUtil.doGet(userInfoUrl, HashMap.class);
            //node_id,用来唯一标识用户
            session.setAttribute("node_id", respUserMap.get("node_id"));
            //github名
            session.setAttribute("nickname", respUserMap.get("login"));
            /*github回调的头像url地址大概率加载不出(加载慢)，不使用github账号头像图片*/
            //session.setAttribute("avatar",respUserMap.get("avatar_url")); //github头像URL
            ...
            UserGithub githubUserInfo = userGithubMapper.findByNodeId(MapUtil.getStr(respUserMap, "node_id"));
            ...
            if (githubUserInfo == null) {
                ...
                github.setCreatedTime(MapUtil.getStr(respUserMap, "created_at"));
                github.setNickname(MapUtil.getStr(respUserMap, "login"));
                github.setIndexUrl(MapUtil.getStr(respUserMap, "html_url"));
                github.setNodeId(MapUtil.getStr(respUserMap, "node_id"));
                github.setPublicRepos(MapUtil.getStr(respUserMap, "public_repos"));
                github.setNickname(MapUtil.getStr(respUserMap, "login"));
                github.setSubscriptions(MapUtil.getStr(respUserMap, "subscriptions_url"));
                github.setUpdatedTime(MapUtil.getStr(respUserMap, "updated_at"));
                github.setGithubId(MapUtil.getStr(respUserMap, "id"));
                github.setReceivedEventsUrl(MapUtil.getStr(respUserMap, "received_events_url"));
